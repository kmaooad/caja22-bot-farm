...
public class UpdateDepartmentStatusCommandHandler implements CommandHandler {
    ...
    private void performStateAction(UserRequest userRequest, UserState userState) {
        final UpdateDepartmentStatusState currentState = (UpdateDepartmentStatusState) userState.getCommandState();
        ...
        switch (currentState) {
            case WAITING_FOR_UPDATE_STATUS_DEPARTMENT_ID:
                ...
                if(depService.getDepartmentById(userInput).isPresent()){
                    ...
                    userState.setCommandState(UpdateDepartmentStatusState.WAITING_FOR_UPDATE_STATUS_DEPARTMENT_ACTION);
                    telegramService.sendMessage(chatId, UpdateDepartmentStatusState.WAITING_FOR_UPDATE_STATUS_DEPARTMENT_ACTION.getMessage() + " Department: " + dep.getName());
                    userStateService.setStateForUser(chatId, userState);
                } else {
                    telegramService.sendMessage(chatId, "Department with provided id doesn't exists! Please, provide a valid id");
                }
                break;
            case WAITING_FOR_UPDATE_STATUS_DEPARTMENT_ACTION:
                ...
                if(Objects.equals(userInput, "+")){
                    ...
                    userState.clear();
                    userStateService.setStateForUser(chatId, userState);
                    telegramService.sendMessage(chatId, "Successfully updated the status of the department! Now the department is open for hire!");
                    ...
                    userState.clear();
                    userStateService.setStateForUser(chatId, userState);
                    telegramService.sendMessage(chatId, "Successfully updated the status of the department! Now the department is closed for hiring!");
                } else {
                    telegramService.sendMessage(
                            chatId, "Value for hiring status is not valid! Please, provide a valid value (+ or -) for the hiring status");
                }
                break;
        }
    }
    ...
    private void initializeUserState(UserRequest userRequest, UserState userState) {
        ...
        userStateService.setStateForUser(chatId, userState);
        telegramService.sendMessage(
                chatId, UpdateDepartmentStatusCommandHandler.UpdateDepartmentStatusState.WAITING_FOR_UPDATE_STATUS_DEPARTMENT_ID.getMessage());
    }
