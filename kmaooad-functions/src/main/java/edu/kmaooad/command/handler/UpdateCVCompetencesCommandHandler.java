...
public class UpdateCVCompetencesCommandHandler implements CommandHandler {
    ...
    private void performStateAction(UserRequest userRequest, UserState userState) {
        final UpdateCVCompetencesState currentState = (UpdateCVCompetencesState) userState.getCommandState();
        ...
        switch (currentState) {
            ...
            case WAITING_FOR_UPDATE_COMPETENCES_CV_ID:
                ...
                if (cvService.getCVById(userInput).isPresent()) {
                    ...
                    userState.setCommandState(UpdateCVCompetencesState.WAITING_FOR_UPDATE_COMPETENCES_CV_ACTION);
                    telegramService.sendMessage(chatId, UpdateCVCompetencesState.WAITING_FOR_UPDATE_COMPETENCES_CV_ACTION.getMessage() + "Your competences:\n\n" + cv.getCompetences());
                    userStateService.setStateForUser(chatId, userState);
                } else {
            ...
            case WAITING_FOR_UPDATE_COMPETENCES_ADD:
                ...
                List<String> competences = Stream.concat(cv.getCompetences().stream(), Stream.of(inputs.get("add")))
